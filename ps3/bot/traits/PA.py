...
class PA(Trait):
    ...
    def get_or_create_room(self, patron, create_room=True):
        ...
        if len(rows) == 1:
            ...
            print(
                "PA: Inviting %s" % patron.email,
                Matrix.invite_threepid_user(
                    self.get_client(), room.room_id, patron.email
                ),
            )
        ...
        if room is not None:
            ...
            for event in Matrix.stream_messages(self.get_client(), room.room_id):
                if (
                    ...
                    room.leave()
                    return event["sender"]
                if event["type"] == "m.room.third_party_invite" or (
                    "membership" in event and event["membership"] == "invite"
                ):
                    invite_worked = True
            if not invite_worked:
                print("We didn't actually invite %s, retrying" % patron.email)
                print(
                    "PA: Reinviting %s" % patron.email,
                    Matrix.invite_threepid_user(
                        self.get_client(), room.room_id, patron.email
                    ),
                )
