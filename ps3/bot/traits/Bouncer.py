...
class Bouncer(Trait):
    ...
    def enforce_group(
        self, group, guests, actually_invite=False, actually_kick=False, verbose=False
    ):
        print()
        ...
        members_and_invited_members = community.members(
            access_token
        ...
        for guest in guests:
            if guest not in members_and_invited_members:
                if actually_invite:
                    if verbose:
                        print(f"Bouncer: Inviting {guest} to {group}")
                    else:
                        print("+%s" % guest)
                    try:
                    ...
                    if verbose:
                        print(f"Bouncer: Would have invited {guest} to {group}")
                    else:
                        print("+%s" % guest)
        ...
        for member in members_and_invited_members:
            if member not in guests:
                if actually_kick and member != "@patroniser:matrix.org":
                    if verbose:
                        print(f"Bouncer: Kicking {member} from {group}")
                    else:
                        print("-%s" % member)
                    try:
                    ...
                    if verbose:
                        print(f"Bouncer: Would have kicked {member} from {group}")
                    else:
                        print("-%s" % member)
    ...
    def enforce(
        ...
        for user in to_kick:
            if user != self.get_client().user_id:
                if actually_kick:
                    if verbose:
                        print(f"Bouncer: Kicking {user} from {room_alias}")
                    else:
                        print("-%s" % user)
                    try:
                        room.kick_user(
                            user,
                            reason="PatroniserBot thinks you're no longer entitled to access this room. If PatroniserBot is wrong, kindly let us know in #matrix:matrix.org. Thanks for your support!",
                        )
                    except KeyError as e:
                        if e.message == "retry_after_ms":
                            if verbose:
                                print(
                                    "Bouncer: Failed to kick %s from %s - remote server 429ed us"
                                    % (user, room_alias)
                                )
                else:
                    if verbose:
                        print(f"Bouncer: Would have kicked {user} from {room_alias}")
                    else:
                        print("-%s" % user)
        ...
        for user in to_invite:
            if (user, room.room_id) not in already_invited:
                if actually_invite:
                    print(f"Bouncer: Inviting {user} to {room_alias}")
                    try:
                        ...
                        if e.message == "retry_after_ms":
                            if verbose:
                                print(
                                    "Bouncer: Failed to invite %s to %s - remote server 429ed us"
                                    % (user, room_alias)
                                )
                    ...
                    if verbose:
                        print(f"Bouncer: Would have invited {user} to {room_alias}")
                    else:
                        print("+%s" % user)
            else:
                if verbose:
                    print(
                        "Bouncer: Politely not re-inviting %s to %s"
                        % (user, room_alias)
                    )
